# 핸들 페이지 views Simple Analytics 설계

## 목표

공개 핸들 페이지의 오늘·어제 pageviews를 Simple Analytics에서 조회해 표시한다. 분석 원본 데이터는 Grabbin 데이터베이스에 저장하지 않는다.
...
- views 조회는 Simple Analytics Stats API의 `fields=pageviews`와 `pages` 필터를 사용한다.
- 날짜는 브라우저 시간대의 오늘·어제로 계산해 `start`, `end`, `timezone`으로 전달한다.
- 조회 결과는 서버에서 15분 캐시한다.
- API 키가 설정된 경우 서버 요청의 `Api-Key` 헤더로만 사용한다.
- 페이지 소유자가 Pro 권한을 가진 경우에만 views 기능을 활성화한다.
- Free 페이지는 views 문구와 로딩 스켈레톤을 모두 표시하지 않는다.
...
6. Pro 페이지에서만 응답의 `pageviews`를 검증하고 오늘·어제 값을 화면과 Tooltip에 전달한다.
...
- [ ] Free 페이지는 views 기능을 표시하거나 조회하지 않는다.
