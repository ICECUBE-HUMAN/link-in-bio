...
1차 성공 기준은 편집 화면에서 드래그, 충돌 처리, 세로 압축, 아이템 생성 위치가 실제 사용 가능한 수준으로 안정화되는 것이다. 자동 스크롤, 별도 drag intent 임계값, custom placeholder 렌더링은 이번 범위에 포함하지 않는다.
...
관련 없는 UI redesign이나 자동 스크롤·custom drag intent 구현은 추가하지 않는다.
...
이번 문서에서 정의하지 않은 자동 스크롤 loop, drag intent threshold, custom placeholder는 별도 후속 범위다.
