...
const avatarProfiles = TESTIMONIAL_PROFILES.slice(0, 11).map(
	({ image, url }) => ({
		image,
		url,
	}),
);
const testimonialSlots = Array.from(
	{ length: TESTIMONIAL_TOTAL_SLOTS },
	(_, slot) => ({
		id: `testimonial-slot-${slot}`,
		slot,
	}),
);
