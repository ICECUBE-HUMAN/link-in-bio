...
import { updatePage } from "@/lib/client/page-api";
import {
  getClientImageUrl,
  uploadPageImage,
...
export function ProfileEditor({
  ...
  const committedRef = useRef({
    ...
    imageSource: initialPage.imageSource,
    imageCrop: initialPage.imageCrop,
  });
  ...
  async function commit(field: "name" | "bio", value: string) {
    if (savedValues.current[field] === value || saving) return;
    setSaving(field);
    setError(null);
    try {
      const result = await updatePage(page.handle, { [field]: value });
  ...
  async function applyCrop(nextCrop: ProfileImageCrop) {
    ...
    setError(null);
    try {
      const result = await updatePage(page.handle, { imageCrop: nextCrop });
      committedRef.current = {
        image: result.page.image,
        imageSource: result.page.imageSource,
        imageCrop: result.page.imageCrop,
  ...
  async function removeImage() {
    ...
    setError(null);
    try {
      const result = await updatePage(page.handle, {
        image: null,
        imageSource: null,
        imageCrop: null,
      });
    ...
    <>
      ...
      >
        <textarea
          ...
          onBlur={(event) => void commit("name", event.target.value)}
          disabled={saving !== null}
        />
        <textarea
          aria-label="Bio"
          ...
          onChange={(event) =>
            setPage((current) => ({ ...current, bio: event.target.value }))
          }
          onBlur={(event) => void commit("bio", event.target.value)}
          disabled={saving !== null}
        />
